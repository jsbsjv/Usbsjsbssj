...
DEFAULTUSERBIO = (
    str(DEFAULT_BIO)
    if DEFAULT_BIO
    else "الحمد لله دائماً وابداً : @WWEEHHHH"
)
...
async def _(event):
    ...
    if replied_user.id == 705475246:
        return await edit_delete(event, "**لا تحاول تنتحل المطورين قشمر**")
...
async def reda(event):
    if event.is_group or event.is_channel:
        ...
        if len(messageo) != 8:
            return await edit_delete(event, "لا تغير الرسالة @WWEEHHHH")
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID,
