...
async def spam_function(event, jepthon, jepiq, sleeptimem, sleeptimet, DelaySpam=False):

    counter = int(jepiq[0])
    if len(jepiq) == 2:
        spam_message = str(jepiq[1])
...
@jepiq.ar_cmd(pattern="كرر (.*)")
async def spammer(event):
    jepthon = await event.get_reply_message()
    jepiq = ("".join(event.text.split(maxsplit=1)[1:])).split(" ", 1)
    try:
        counter = int(jepiq[0])
    except Exception:
...
@jepiq.ar_cmd(pattern="ايقاف التكرار ?(.*)")
